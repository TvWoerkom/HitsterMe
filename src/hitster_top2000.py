...
################################
tracks_fn = Path('Top2000/SpotifyTop2000_2024.csv')
tracks_df = pd.read_csv(tracks_fn, index_col = 0)

qr_folder = tracks_fn.parent / 'qrs'
...
#%% make pdf
# Constants
FOLDER_X = qr_folder  # Folder containing QR codes
OUTPUT_PDF = f"{qr_folder.parent}\output.pdf"  # Output PDF file
...
#do track selection to a relatively equal spread over index position
total_cards = 180

# Divide tracks into equal slices and select one random track from each slice
tracks_df_sorted = tracks_df.sort_index()
slice_size = len(tracks_df_sorted) // total_cards
selection_indices = []

for i in range(total_cards):
    start_idx = i * slice_size
    if i == total_cards - 1:  # Last slice gets remaining tracks
        end_idx = len(tracks_df_sorted)
    else:
        end_idx = (i + 1) * slice_size
    
    slice_tracks = tracks_df_sorted.iloc[start_idx:end_idx]
    random_track_idx = np.random.choice(slice_tracks.index)
    selection_indices.append(random_track_idx)

selection = tracks_df.loc[selection_indices]
selection.index.name = 'positie'
selection.reset_index(inplace=True)
selection['jaar'] = '(' + selection['jaar'].astype(str) + ')'
...
for page_start in range(0, total_cards, codes_per_page):
    ...
    for idx, qr_filename in enumerate(page_tracks.positie):
    ...
    for idx, (track_idx, track) in enumerate(page_tracks.iterrows()):
        ...
        text_lines = track[['titel', 'artiest', 'positie', 'jaar']].values.astype(str)
